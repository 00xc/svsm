...
//! hypervisor-shared).
//!
//! The module contains two types of mappings: [`Mapping`] owns a
//! memory mappping, and unmaps the backing memory on drop.
//! [`MappingRef`] does not drop any mappings, and has a lifetime
//! to indicate the validity of such mapping — this is typically the
//! lifetime of the [`Mapping`] from which the borrow was acquired.
//!
//! Both mappings implement common interfaces through the following traits:
//!
//! * [`Mapping`]: common to all types, allows creating [`MappingRef`]s
//!   from other mappings.
...
pub trait BorrowMapping<A, T: ?Sized> {
    fn borrow<U>(&self) -> Result<MappingRef<'_, A, U>, SvsmError> {
        self.borrow_at(0)
    }
    ...
    fn borrow_slice<U>(&self, len: usize) -> Result<MappingRef<'_, A, [U]>, SvsmError> {
...
impl<A, T, M> BorrowMapping<A, T> for &M
where
    T: ?Sized,
    M: BorrowMapping<A, T>,
{
...
impl<A, T, M> BorrowMapping<A, T> for &mut M
where
    T: ?Sized,
    M: BorrowMapping<A, T>,
{
...
pub trait ReadableMapping<A: ReadAccess, T: FromBytes>: BorrowMapping<A, T> {
...
pub trait WriteableMapping<A: WriteAccess, T: IntoBytes>: BorrowMapping<A, T> {
...
pub trait ReadableSliceMapping<A, T>: BorrowMapping<A, T>
where
    A: ReadAccess,
    T: Index<usize> + ?Sized,
...
pub trait WriteableSliceMapping<A, T>: BorrowMapping<A, T>
where
    A: WriteAccess,
    T: Index<usize> + ?Sized,
