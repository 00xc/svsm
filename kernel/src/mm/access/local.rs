...
impl<T> Mapping<Local, T> {
    ...
    pub unsafe fn map(paddr: PhysAddr) -> Result<Self, SvsmError> {
...
impl<T> Mapping<Local, [T]> {
    ...
    /// the SVSM kernel.
    pub unsafe fn map(paddr: PhysAddr, len: usize) -> Result<Mapping<Local, [T]>, SvsmError> {
        Self::map_inner::<false>(paddr, len)
    }
}

impl<T: FromBytes> Mapping<Local, T> {
...
impl<T: FromBytes + IntoBytes> Mapping<Local, [T]> {
    /// # Safety
    ///
...
impl<T: FromBytes> MappingRef<'_, Local, T> {
    /// Gets a reference to the underlying `T`. This method is only available
    /// for SVSM-local mappings.
    ///
    /// # Safety
...
impl<T: FromBytes + IntoBytes> MappingRef<'_, Local, T> {
    /// Gets a reference to the underlying `T`. This method is only available
    /// for SVSM-local mappings.
    ///
    /// # Safety
