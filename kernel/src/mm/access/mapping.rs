...
use super::{
    BorrowMapping, ReadAccess, ReadableMapping, ReadableSliceMapping, WriteAccess,
    WriteableMapping, WriteableSliceMapping,
};
...
impl<A, T: ?Sized> Mapping<A, T> {
    ...
    fn borrow_at<U>(&self, byte_off: usize) -> Result<MappingRef<'_, A, U>, SvsmError> {
        let start = self
            .mem
            .region()
            .start()
    ...
    fn borrow_slice_at<U>(
        ...
        byte_off: usize,
        len: usize,
    ) -> Result<MappingRef<'_, A, [U]>, SvsmError> {
        let start = self
            .mem
            .region()
            .start()
...
impl<A, T> Mapping<A, T> {
    /// Returns the equivalent [`MappingRef`]. This is used to implement
    /// trait methods reusing [`MappingRef`] functionality.
    #[inline]
    pub(super) fn as_borrow(&self) -> MappingRef<'_, A, T> {
        MappingRef {
    ...
    pub(super) fn map_inner<const SHARED: bool>(paddr: PhysAddr) -> Result<Self, SvsmError> {
...
impl<A, T> Mapping<A, [T]> {
    ...
    pub(super) fn map_inner<const SHARED: bool>(
        paddr: PhysAddr,
        len: usize,
    ) -> Result<Self, SvsmError> {
        let off = paddr.page_offset();
        let mem = Mapping::<A, T>::map_common::<SHARED>(paddr, len)?;
        Ok(Mapping {
            mem,
            off,
            len,
...
/// for example this structure is created from [`Mapping::borrow()`], this
...
impl<A, T> MappingRef<'_, A, T> {
    ...
    pub unsafe fn from_raw<V>(addr: V) -> Result<Self, SvsmError>
    where
        V: Into<VirtAddr>,
    {
        let addr = addr.into();
...
impl<A, T> MappingRef<'_, A, [T]> {
    ...
    /// they must also ensure that it does not exceed the lifetime of the actual
    /// mapping in the page table.
    pub unsafe fn from_raw<V>(addr: V, len: usize) -> Result<Self, SvsmError>
    where
        V: Into<VirtAddr>,
    {
        let addr = addr.into();
...
impl<A, T> BorrowMapping<A, T> for Mapping<A, T>
where
    T: ?Sized,
{
    fn borrow_at<U>(&self, byte_off: usize) -> Result<MappingRef<'_, A, U>, SvsmError> {
...
impl<A, T> BorrowMapping<A, T> for MappingRef<'_, A, T>
where
    T: ?Sized,
{
    fn borrow_at<U>(&self, byte_off: usize) -> Result<MappingRef<'_, A, U>, SvsmError> {
...
impl<A, T> ReadableMapping<A, T> for Mapping<A, T>
where
    A: ReadAccess,
    T: FromBytes,
{
...
impl<A, T> ReadableMapping<A, T> for MappingRef<'_, A, T>
where
    A: ReadAccess,
    T: FromBytes,
{
...
impl<A, T> WriteableMapping<A, T> for Mapping<A, T>
where
    A: WriteAccess,
    T: IntoBytes,
{
...
impl<A, T> WriteableMapping<A, T> for MappingRef<'_, A, T>
where
    A: WriteAccess,
    T: IntoBytes,
{
...
impl<A, T> ReadableSliceMapping<A, [T]> for Mapping<A, [T]>
where
    A: ReadAccess,
    T: FromBytes,
{
    fn read_item(&self, idx: usize) -> Result<T, SvsmError> {
        self.as_borrow().read_item(idx)
    }

    fn read_to(&self, dst: &mut [T]) -> Result<(), SvsmError> {
        self.as_borrow().read_to(dst)
    }

    fn read_to_vec(&self) -> Result<Vec<T>, SvsmError> {
        self.as_borrow().read_to_vec()
    }
}

impl<A, T> ReadableSliceMapping<A, [T]> for MappingRef<'_, A, [T]>
where
    A: ReadAccess,
    T: FromBytes,
{
...
impl<A, T> WriteableSliceMapping<A, [T]> for Mapping<A, [T]>
where
    A: WriteAccess,
    T: IntoBytes,
{
    fn write_item<B: Borrow<T>>(&mut self, val: B, idx: usize) -> Result<(), SvsmError> {
        self.as_borrow().write_item(val, idx)
    }

    fn write_from(&mut self, src: &[T]) -> Result<(), SvsmError> {
        self.as_borrow().write_from(src)
    }
}

impl<A, T> WriteableSliceMapping<A, [T]> for MappingRef<'_, A, [T]>
where
    A: WriteAccess,
    T: IntoBytes,
{
...
mod test {
    ...
    fn get_test_page() -> MappingRef<'static, Local, DataPage> {
        ...
        unsafe { MappingRef::<_, DataPage>::from_raw(addr).unwrap() }
    }
    ...
    fn test_read() {
        let mapping = get_test_page();
        assert_eq!(
            BorrowMapping::borrow::<u8>(&mapping)
                .unwrap()
                .read()
                .unwrap(),
            0xfe
