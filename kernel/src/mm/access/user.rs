...
impl<T> MappingRef<'_, User, T> {
    ...
    pub fn from_raw_checked<V>(addr: V) -> Result<Self, SvsmError>
    where
        V: Into<VirtAddr>,
    {
        let region = Self::checked_region(addr.into(), 1)?;
        Ok(Self {
            region,
...
impl<T> MappingRef<'_, User, [T]> {
    ...
    /// memory region (starting at `addr`) is within the userspace region
    /// of the virtual address space.
    pub fn from_raw_checked<V>(addr: V, len: usize) -> Result<Self, SvsmError>
    where
        V: Into<VirtAddr>,
    {
        let region = MappingRef::<User, T>::checked_region(addr.into(), len)?;
        Ok(MappingRef {
            region,
            _phantom1: PhantomData,
            _phantom2: PhantomData,
