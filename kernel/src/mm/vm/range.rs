...
/// Granularity of ranges mapped by a [`Vmr`]. The mapped region of a [`Vmr`]
/// is always a multiple of this constant. One [`VMR_GRANULE`] covers one
/// top-level page-table entry on x86-64 with 4-level paging.
...
pub trait VmRange: Sized + core::fmt::Debug {
...
/// faults.
pub trait VmAllocator<V: VmRange>: core::fmt::Debug {
    ...
    /// Releases the reservation based at `base`, returning its [`Mapping`].
...
impl<V: VmRange> Vmr<V> {
    ...
    pub fn remove(&self, base: VirtAddr) -> Result<Mapping, SvsmError> {
        let mapping = self.alloc.free(base).ok_or(SvsmError::Mem)?;
        self.unmap_mapping(base, &mapping);
