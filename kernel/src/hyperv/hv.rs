...
use crate::utils::percpu_cell::PerCpuRefMut;
...
struct HypercallInput<'a, 'b, H, T> {
    page: &'a mut PerCpuRefMut<'b, HypercallPage>,
    rep_count: usize,
...
impl<'a, 'b, H, T> HypercallInput<'a, 'b, H, T> {
    fn new(page: &'a mut PerCpuRefMut<'b, HypercallPage>) -> Self {
    ...
    fn new_rep(page: &'a mut PerCpuRefMut<'b, HypercallPage>) -> Self {
        const { assert!(size_of::<H>() <= PAGE_SIZE) };
...
struct HypercallOutput<'a, 'b, T> {
    page: &'a PerCpuRefMut<'b, HypercallPage>,
    rep_count: usize,
...
impl<'a, 'b, T> HypercallOutput<'a, 'b, T> {
    ...
    unsafe fn new(page: &'a PerCpuRefMut<'b, HypercallPage>, rep_count: usize) -> Self {
        Self {
            page,
            rep_count,
...
pub struct HypercallPagesGuard<'a> {
    pub input: PerCpuRefMut<'a, HypercallPage>,
    pub output: PerCpuRefMut<'a, HypercallPage>,
    _irq_guard: IrqGuard,
}
...
impl<'a> HypercallPagesGuard<'a> {
    ...
    /// is wrapped in a [`PerCpuRefMut`], meaning that there is a runtime guarantee
    /// that nobody else holds a reference to the same pages.
    pub fn new(page_ref: PerCpuRefMut<'a, (HypercallPage, HypercallPage)>) -> Self {
        let (input, output) = PerCpuRefMut::map_split(page_ref, |r| (&mut r.0, &mut r.1));
        Self {
            input,
            output,
