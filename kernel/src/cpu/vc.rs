...
use crate::mm::access::{GuestMappingRef, ReadableMapping};
...
fn vc_decode_insn(ctx: &X86ExceptionContext) -> Result<Option<DecodedInsnCtx>, SvsmError> {
    ...
    // To ensure this, we rely on GuestMappingRef::read(), which uses the
    ...
    // safely be read.
    let insn_raw =
        unsafe { GuestMappingRef::<'_, [u8; MAX_INSN_SIZE]>::from_raw(ctx.frame.rip)?.read()? };
