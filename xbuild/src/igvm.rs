...
impl IgvmTargetConfig {
    ...
    fn igvmbuild(&self, args: &Args, target: IgvmTarget, parts: &RecipeParts) -> BuildResult<()> {
    ...
    fn igvmmeasure(&self, args: &Args) -> BuildResult<()> {
        ...
        cmd.arg(&self.output).arg(self.measure.as_arg());
        run_cmd_checked(cmd, args)
    }

    fn build(&self, args: &Args, target: IgvmTarget, parts: &RecipeParts) -> BuildResult<()> {
        self.igvmbuild(args, target, parts)?;
        self.igvmmeasure(args)?;
        Ok(())
    }
}
